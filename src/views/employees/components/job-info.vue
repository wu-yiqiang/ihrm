...
输入"
      ...
      <el-form-item>
        ...
        <el-button @click="$router.back()">返回</el-button> </el-form-item
      >定义岗位数据 在detail.vue组件中，注册并使用 本节任务
      ：完成个人组件和岗位组件封装
    </el-form>
  </div>
</template>
...
import {getEmployeesList, getJobDetail,updateJob} from '@/api/employees'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    async getEmployeesList () {
      this.depts = await getEmployeesList()
    },
    async saveJob () {
